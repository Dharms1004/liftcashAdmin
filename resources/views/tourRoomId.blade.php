                        ...
                        <div class="col-md-12">
                            ...
                            <div class="card card-primary">
                                <div class="card-header">
                                    ...
                                    @else
                                    <h3 class="card-title">Add Tournament Room</h3>
                                    @endif
                                </div>
                                <!-- /.card-header -->
                                <!-- form start -->
                                <form method="POST" action="{{ route('addRoom') }}" enctype="multipart/form-data">
                                    @csrf
                                    <div class="card-body">
                                        <input type="hidden" name="tour_id" value="{{ $tourData }}">
                                        <div class="form-group">
                                            <label for="exampleInputEmail1">Room Id</label>
                                            <input type="text" name="room_id" value="@if(!empty(old('room_id'))) {{ old('room_id') }} @else {{ !empty($gameData->ROOM_ID) ? $gameData->ROOM_ID : ''  }} @endif" class="form-control" i placeholder="Room Id">
                                        </div>
                                        <div class="form-group">
                                            <label for="exampleInputEmail1">Password</label>
                                            <input type="text" name="pass" value="@if(!empty(old('tourData'))) {{ old('tourData') }} @else {{ !empty($gameData->PASSWORD) ? $gameData->PASSWORD : ''  }} @endif" class="form-control" i placeholder="Password">
                                        </div>
                                    </div>
