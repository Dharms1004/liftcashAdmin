                        ...
                        <div class="col-md-12">
                            ...
                            <div class="card card-primary">
                                ...
                                <form method="POST" action="{{ route('createCountry') }}" enctype="multipart/form-data">
                                    ...
                                    <input type="hidden" name="ID" value="{{ !empty($countryData->ID) ? $countryData->ID : ''  }}">
                                    @endif
                                    <div class="card-body">

                                        <div class="form-group">
                                            <label for="exampleInputEmail1">Country Name</label>
                                            <input type="text" name="name" value="@if(!empty(old('name'))) {{ old('name') }} @else {{ !empty($countryData->NAME) ? $countryData->NAME : ''  }} @endif" class="form-control" i placeholder="Country Name">
                                        </div>

                                        <div class="form-group">
                                            <label for="exampleInputFile">Country Flag</label>
                                            <div class="input-group">
                                                <div class="custom-file">
                                                    <input type="file" accept="image/*" name="FLAG" value="{{ old('image') }} {{ !empty($countryData->FLAG) ? $countryData->FLAG : ''  }}" class="custom-file-input">
                                                    <label class="custom-file-label" for="exampleInputFile">Choose file</label>
                                                </div>
                                            </div>
                                            @if(!empty($countryData->FLAG))
                                            <img style="height:50px; width:100px" src="{{url('images/country/'.$countryData->FLAG)}}">
                                            @endif
                                        </div>
                                    </div>
